...
assert.match(product,/<div id="root"><\/div>/); assert.match(product,/assets\/index-/);
assert.ok(!product.includes('FUNCTIONAL MATERIALS · CHINA'), 'route entry must not replace the React site with the legacy fallback HTML');
